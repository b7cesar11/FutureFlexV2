  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center sm:justify-center">
      ...
      <div
        ...
        className="relative z-10 w-full sm:max-w-lg max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-xl border border-white/10 bg-[#0d0d0f] p-6"
      >
        <div className="mb-5 flex items-start justify-between">
          <div>
            <p className="label-caps">Ação rápida</p>
            <h3 className="mt-1 text-xl font-semibold">Registrar</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-3">
          ...
          {type === "third_party" && (
            <>
              ...
              <select
                className={field}
                data-testid="quick-add-direction"
                value={form.direction || "receivable"}
              ...
              <input
                ...
                inputMode="numeric"
                data-testid="quick-add-tp-installments"
                value={form.installments || ""}
                onChange={set("installments")}
              />
            </>
          ...
          {(isCommitment || type === "third_party") && (
            <select
              ...
              onChange={set("credit_card_id")}
            >
              <option value="">Sem cartão (conta / dinheiro)</option>
              {cards.map((c) => (
          ...
          {type === "transfer" && (
            <select
              className={field}
              data-testid="quick-add-to-account"
          ...
          >
            ...
            {categories
              .filter((c) =>
                ["income", "recurring_income"].includes(type) ? c.kind === "income" : c.kind === "expense",
              )
              .map((c) => (
                <option key={c.id} value={c.id}>
                  {c.name}
        ...
        <button
          ...
          className="mt-6 w-full rounded-md bg-[#ccff00] py-3 text-sm font-semibold text-black transition-colors hover:bg-[#b3e600] disabled:opacity-50"
        >
          {mutation.isPending ? "Registrando..." : "Registrar"}
        </button>
        {isCommitment && (
          <p className="mt-3 text-center text-[11px] text-zinc-500">
            Compromissos alimentam automaticamente os próximos 24 meses.
          </p>
        )}
      </div>
    </div>
